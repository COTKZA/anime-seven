...
const Edit_Tags = () => {
  ...
  const handleUpdate = async (event: FormEvent) => {
    ...
    try {
      setLoading(true)
      const res = await axios.put(`${import.meta.env.VITE_API_URL}/tags/${id}`, { name })
  ...
  return (
    <AdminContainer>
      <div className="bg-[#e64a45] rounded-t-sm p-3">
        <h1 className="text-white font-extrabold text-xl">แก้ไขแท็ก</h1>
      </div>
        ...
        <div>
          <form onSubmit={handleUpdate}>
            ...
            <div className="flex justify-end items-center mt-5 gap-3">
              ...
              <button type="submit" className="px-5 py-2 bg-red-500 hover:bg-red-600 rounded-xl text-white" disabled={loading}>บันทึก</button>
            </div>
          </form>
        </div>
      </div>
